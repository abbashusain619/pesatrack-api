...
<div class="container mx-auto px-4 py-8 max-w-md">
    ...
    <form method="POST" action="{{ route('categories.update', $category) }}" class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="color" name="color" value="{{ old('color', $category->color) }}" class="w-full border rounded px-3 py-2">
            @error('color') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Icon (emoji or text)</label>
            <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" placeholder="🍔" class="w-full border rounded px-3 py-2">
